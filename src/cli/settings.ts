...
export function handleSettingsCommand(args: string[]): void {
  const sub = args[0];
  switch (sub) {
    case 'get': return settingsGet(args[1]);
    case 'set': return settingsSet(args[1], args[2]);
    case 'list': return settingsList();
    default:
...
function settingsGet(key: string): void {
  if (!key) { console.error('Usage: osd settings get <key>'); process.exit(1); }
  const db = initDatabase(DB_PATH);
  const val = getSetting(db, key);
  db.close();
  if (val === undefined) { console.log(`(not set)`); } else { console.log(val); }
}

function settingsSet(key: string, value: string): void {
  if (!key || value === undefined) { console.error('Usage: osd settings set <key> <value>'); process.exit(1); }
  const db = initDatabase(DB_PATH);
...
function settingsList(): void {
  ...
  db.close();
  if (rows.length === 0) { console.log('No settings configured.'); return; }
  console.log('\nSettings:\n');
