/**
 * Sidebar — Slack-style 48px icon strip on the left edge.
 */
...
export function setupSidebar(mainWindow: BrowserWindow): void {
  ...
  sidebarView.webContents.loadURL('about:blank');
  sidebarView.webContents.on('did-finish-load', () => {
    sidebarView!.webContents.executeJavaScript(`
      document.body.innerHTML = ${JSON.stringify(SIDEBAR_HTML)};
...
const SIDEBAR_HTML = `<style>
* { margin:0; padding:0; box-sizing:border-box; }
body { width:48px; height:100vh; background:#1b1b2f; display:flex; flex-direction:column; align-items:center; padding:12px 0; gap:8px; font-family:-apple-system,system-ui,sans-serif; -webkit-app-region:drag; }
.btn { width:36px; height:36px; border-radius:8px; border:none; background:transparent; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background 0.15s; -webkit-app-region:no-drag; }
...
function nav(target) {
  ...
  if (target === 'home') ipcRenderer.invoke('osd:navigate', '/app/home');
    ...
    ipcRenderer.invoke('osd:bounce').then(() => event.currentTarget.classList.remove('active'));
  }
