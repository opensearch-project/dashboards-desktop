...
export class ConversationManager {
  ...
  getMessages(conversationId: string): ChatMessage[] {
    ...
    ).all(conversationId) as Array<{ role: string; content: string; tool_calls: string | null; tool_call_id: string | null }>;
  ...
  buildContext(
    ...
    const rows = this.db.prepare(
      ...
      role: string; content: string; tool_calls: string | null; tool_call_id: string | null; token_count: number | null;
    }>;
