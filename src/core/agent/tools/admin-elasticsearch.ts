...
export const adminElasticsearchTool: AgentTool = {
  definition: {
    ...
    description: 'Manage Elasticsearch cluster: ILM policies, Watcher alerts, snapshots, ingest pipelines, security (users/roles/API keys).',
    source: 'builtin',
    inputSchema: {
      type: 'object',
      properties: {
        domain: { type: 'string', enum: ['ilm', 'watcher', 'snapshots', 'ingest', 'security'] },
        action: { type: 'string', enum: ['list', 'get', 'create', 'update', 'delete', 'restore', 'activate', 'deactivate'] },
  ...
  async execute(input: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      switch (input.domain) {
        case 'ilm':
          res = action === 'list' ? await ilm.listPolicies(url)
            : action === 'get' ? await ilm.getPolicy(url, name)
            : action === 'create' ? await ilm.createPolicy(url, name, body)
            : action === 'delete' ? await ilm.deletePolicy(url, name)
      ...
      return { content: `admin-elasticsearch failed: ${err instanceof Error ? err.message : err}`, isError: true };
    }
  },
};

async function execWatcher(url: string, action: string, name: string, body: Record<string, unknown>) {
  switch (action) {
    case 'list': return watcher.listWatches(url);
    case 'get': return watcher.getWatch(url, name);
    case 'create': return watcher.createWatch(url, name, body);
    case 'delete': return watcher.deleteWatch(url, name);
    case 'activate': return watcher.activateWatch(url, name);
    case 'deactivate': return watcher.deactivateWatch(url, name);
    default: return `Unknown watcher action: ${action}`;
  }
}
...
async function execSecurity(url: string, action: string, name: string, body: Record<string, unknown>) {
  switch (action) {
    case 'list': return security.listUsers(url);
    case 'get': return security.listRoles(url);
    case 'create': return security.createUser(url, name, body);
    case 'delete': return security.deleteUser(url, name);
