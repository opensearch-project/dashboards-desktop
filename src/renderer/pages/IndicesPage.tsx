...
export const IndicesPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      setIndices(await window.osd.indices.list());
    } catch (e: unknown) {
  ...
  const filtered = search
    ? indices.filter((i) => i.name.toLowerCase().includes(search.toLowerCase()))
    : indices;
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      await window.osd.indices.delete(name);
      load();
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    }
  ...
  const handleReindex = async () => {
    ...
    try {
      await window.osd.indices.reindex(selected, reindexDest);
      setDialog(null);
      load();
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    ...
    <div className="admin-page" role="region" aria-label="Index management">
      <header className="admin-header">
        ...
        <div className="admin-header-actions">
          ...
          />
          <button className="btn-sm" onClick={() => setDialog('alias')}>
            Manage Aliases
          </button>
          <button className="btn-primary btn-sm" onClick={() => setDialog('create')}>
            + Create Index
          </button>
        ...
        <tbody>
          {filtered.map((idx) => (
            <tr key={idx.name}>
              ...
              <td>
                <span className={`health-dot health-${idx.health}`} aria-label={idx.health} />{' '}
                {idx.health}
              </td>
              <td>{idx.status}</td>
              ...
              <td>
                {idx.primary_shards}p / {idx.replica_shards}r
              </td>
              <td>
                {idx.aliases.length
                  ? idx.aliases.map((a) => (
                      <span key={a} className="alias-tag">
                        ...
                        <button
                          ...
                          onClick={() => handleRemoveAlias(idx.name, a)}
              ...
              <td className="action-cell">
                {idx.status === 'open' ? (
                  <button
                    className="btn-sm"
                    onClick={() => window.osd.indices.close(idx.name).then(load)}
                  >
                    Close
                  </button>
                ) : (
                  <button
                    className="btn-sm"
                    onClick={() => window.osd.indices.open(idx.name).then(load)}
                  >
                    Open
                  </button>
                )}
                <button
                  className="btn-sm"
                  onClick={() => {
                    setSelected(idx.name);
                    setDialog('reindex');
                  }}
                >
                  Reindex
                </button>
                <button className="btn-sm btn-danger-sm" onClick={() => handleDelete(idx.name)}>
                  Delete
                </button>
              </td>
            </tr>
      ...
      {filtered.length === 0 && (
        <div className="empty-state" role="status">
          <p>{search ? 'No matching indices' : 'No indices found'}</p>
        </div>
      )}
      ...
      {dialog === 'create' && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="create-idx-title"
          >
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                className="json-editor"
                value={newMappings}
                onChange={(e) => setNewMappings(e.target.value)}
                rows={4}
              />
            </div>
            <div className="dialog-actions">
              ...
              <button className="btn-primary" onClick={handleCreate} disabled={!newName}>
                Create
              </button>
            </div>
          </div>
        ...
        >
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="reindex-title">
            ...
            <div className="dialog-actions">
              ...
              <button className="btn-primary" onClick={handleReindex} disabled={!reindexDest}>
                Reindex
              </button>
            </div>
          </div>
          ...
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="alias-title">
            ...
            <div className="form-group">
              ...
              <select
                id="alias-idx"
                value={aliasIndex}
                onChange={(e) => setAliasIndex(e.target.value)}
              >
