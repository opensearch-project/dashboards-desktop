...
export const PluginsPage: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setPlugins(await window.osd.plugins.list());
    } catch (e: unknown) {
  ...
  const handleInstall = async () => {
    ...
    setError('');
    try {
      await window.osd.plugins.install(installSource.trim());
      ...
      {filtered.length === 0 ? (
        ...
        <div className="plugin-grid" role="list">
          {filtered.map((p) => (
            <div
              key={p.name}
              className={`plugin-card ${!p.enabled ? 'plugin-disabled' : ''}`}
              role="listitem"
            >
              ...
              <div className="plugin-actions">
                <button className="btn-sm" onClick={() => setDetail(p)}>
                  Details
                </button>
                <button className="btn-sm btn-danger-sm" onClick={() => handleUninstall(p.name)}>
                  Uninstall
                </button>
              </div>
            </div>
          ...
          >
            ...
            {detail.changelog && (
              <details>
                ...
                <pre className="plugin-changelog">{detail.changelog}</pre>
              </details>
            )}
          ...
          >
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="opensearch-security-dashboards or ./my-plugin.zip"
              />
            </div>
            <div className="dialog-actions">
