...
export const McpPage: React.FC = () => {
  ...
  if (loading) return <div className="page-loading" role="status">Loading MCP servers…</div>;
    ...
    <div className="admin-page" role="region" aria-label="MCP Servers">
      <header className="admin-header">
        ...
        <div className="admin-header-actions">
          <button className="btn-primary btn-sm" onClick={() => setInstallOpen(true)}>+ Install Server</button>
      ...
      {servers.length === 0 ? (
        <div className="empty-state" role="status">
          ...
          <button className="btn-primary" onClick={() => setInstallOpen(true)}>Install your first MCP server</button>
        </div>
      ) : (
        <table className="admin-table" role="table">
          <thead><tr><th>Name</th><th>Status</th><th>Tools</th><th>Memory</th><th>Actions</th></tr></thead>
          <tbody>
            {servers.map(s => (
              <tr key={s.name}>
                <td><strong>{s.name}</strong></td>
                <td><span className={`health-dot ${statusClass(s.status)}`} aria-label={s.status} /> {s.status}</td>
                <td><button className="btn-link" onClick={() => openTools(s.name)}>{s.tools_count} tools</button></td>
                <td>{s.memory_mb > 0 ? `${s.memory_mb} MB` : '—'}</td>
                <td className="action-cell">
                  {s.status === 'running' ? (
                    <button className="btn-sm" onClick={async () => { await window.osd.mcp.stop(s.name); load(); }}>Stop</button>
                  ) : (
                    <button className="btn-sm" onClick={async () => { await window.osd.mcp.start(s.name); load(); }}>Start</button>
                  )}
                  <button className="btn-sm" onClick={async () => { await window.osd.mcp.restart(s.name); load(); }}>Restart</button>
                  <button className="btn-sm" onClick={() => openConfig(s.name)}>Config</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {installOpen && (
        <div className="dialog-overlay" role="presentation" onClick={e => { if (e.target === e.currentTarget) setInstallOpen(false); }}>
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="install-mcp-title">
            <h2 id="install-mcp-title">Install MCP Server</h2>
            <div className="form-group">
              <label htmlFor="mcp-source">npm package or local path</label>
              <input id="mcp-source" value={installSource} onChange={e => setInstallSource(e.target.value)} placeholder="@modelcontextprotocol/server-filesystem" />
            </div>
      ...
      {/* Config editor */}
      {configServer && (
        <div className="dialog-overlay" role="presentation" onClick={e => { if (e.target === e.currentTarget) setConfigServer(null); }}>
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="mcp-config-title">
            <h2 id="mcp-config-title">Config: {configServer}</h2>
            <div className="form-group">
              <label htmlFor="mcp-config-json">Server Configuration (JSON)</label>
              <textarea id="mcp-config-json" className="json-editor" value={configJson} onChange={e => setConfigJson(e.target.value)} rows={12} />
            </div>
            <div className="dialog-actions">
      ...
      {/* Tools list */}
      {toolsServer && (
        <div className="dialog-overlay" role="presentation" onClick={e => { if (e.target === e.currentTarget) setToolsServer(null); }}>
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="mcp-tools-title">
            ...
            {tools.length === 0 ? <p>No tools exposed</p> : (
              <table className="admin-table" role="table">
            ...
            <div className="dialog-actions"><button className="btn-secondary" onClick={() => setToolsServer(null)}>Close</button></div>
          </div>
        </div>
      )}
    </div>
