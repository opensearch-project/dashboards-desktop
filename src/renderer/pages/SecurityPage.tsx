  ...
  const saveRole = async () => {
    try {
      await window.osd.security.roles.save(roleName, {
        cluster_permissions: roleClusterPerms
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean),
    ...
    try {
      const body: Record<string, unknown> = {
        backend_roles: userBackendRoles
          ...
          .map((s) => s.trim())
          .filter(Boolean),
        opendistro_security_roles: userRoles
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean),
      };
      ...
      <div className="admin-tabs" role="tablist">
        {(['roles', 'users', 'tenants'] as Tab[]).map((t) => (
          ...
          >
            ...
            {t === 'roles' ? roles.length : t === 'users' ? users.length : tenants.length})
          </button>
        ))}
      </div>
        ...
        <section role="tabpanel" aria-label="Roles">
          ...
          <table className="admin-table" role="table">
            <thead>
              <tr>
                <th>Name</th>
                <th>Cluster Permissions</th>
                <th>Index Permissions</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {roles.map((r) => (
                <tr key={r.name}>
                  ...
                  <td>
                    {r.index_permissions.map((p) => p.index_patterns.join(', ')).join('; ') || '—'}
                  </td>
                  <td className="action-cell">
                    <button className="btn-sm" onClick={() => openEditRole(r)}>
                      Edit
                    </button>
                    <button
                      className="btn-sm btn-danger-sm"
                      onClick={async () => {
                        await window.osd.security.roles.delete(r.name);
                        load();
                      }}
                    >
                      Delete
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.username}>
                  ...
                  <td className="action-cell">
                    <button className="btn-sm" onClick={() => openEditUser(u)}>
                      Edit
                    </button>
                    <button
                      className="btn-sm btn-danger-sm"
                      onClick={async () => {
                        await window.osd.security.users.delete(u.username);
                        load();
                      }}
                    >
                      Delete
            ...
            <tbody>
              {tenants.map((t) => (
                <tr key={t.name}>
                  ...
                  <td>{t.description || '—'}</td>
                  <td className="action-cell">
                    <button className="btn-sm" onClick={() => openEditTenant(t)}>
                      Edit
                    </button>
                    <button
                      className="btn-sm btn-danger-sm"
                      onClick={async () => {
                        await window.osd.security.tenants.delete(t.name);
                        load();
                      }}
                    >
                      Delete
        ...
        >
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="role-title">
            ...
            <div className="form-group">
              ...
              <input
                id="sec-role-cluster"
                value={roleClusterPerms}
                onChange={(e) => setRoleClusterPerms(e.target.value)}
                placeholder="cluster_monitor, cluster_composite_ops"
              />
            </div>
            <div className="form-group">
              ...
              <input
                id="sec-role-idx"
                value={roleIndexPatterns}
                onChange={(e) => setRoleIndexPatterns(e.target.value)}
            ...
            <div className="form-group">
              ...
              <input
                id="sec-role-actions"
                value={roleIndexActions}
                onChange={(e) => setRoleIndexActions(e.target.value)}
                placeholder="read, search, indices_monitor"
              />
            </div>
            <div className="dialog-actions">
              ...
              <button className="btn-primary" onClick={saveRole} disabled={!roleName}>
                Save
              </button>
            </div>
          </div>
        ...
        >
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="user-title">
            ...
            <div className="form-group">
              ...
              <input
                id="sec-user-backend"
                value={userBackendRoles}
                onChange={(e) => setUserBackendRoles(e.target.value)}
              />
            </div>
            <div className="dialog-actions">
              <button
              ...
              <button
                ...
                onClick={saveUser}
                disabled={!userName || (!editing && !userPassword)}
              >
                Save
              </button>
            </div>
        ...
        >
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="tenant-title">
            ...
            <div className="dialog-actions">
              ...
              <button className="btn-primary" onClick={saveTenant} disabled={!tenantName}>
                Save
              </button>
            </div>
          </div>
