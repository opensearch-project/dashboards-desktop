...
export const SettingsPage: React.FC = () => {
  ...
  const checkUpdates = async () => {
    ...
    setError('');
    try {
      setUpdateInfo(await window.osd.updates.check());
    } catch (e: unknown) {
  ...
  const installUpdate = async () => {
    try {
      await window.osd.updates.install();
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
    }
  };
        ...
        <section role="tabpanel" aria-label="General settings" className="settings-section">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onBlur={() => saveSetting('defaultModel', defaultModel)}
      ...
      {tab === 'models' && (
        <section role="tabpanel" aria-label="Model providers" className="settings-section">
          <p className="settings-hint">
            Model providers are configured via the model switcher in the chat panel or via CLI:{' '}
            <code>osd chat --model provider:model</code>
          </p>
          <p className="settings-hint">
            Supported: Ollama (local), OpenAI, Anthropic, Amazon Bedrock, any OpenAI-compatible API.
          </p>
          <p className="settings-hint">
            API keys are stored securely in your OS keychain via Electron safeStorage.
          </p>
        </section>
      )}
            ...
            <label className="toggle-label">
              <input
                ...
                onChange={(e) => {
                  ...
                  saveSetting('autoUpdate', String(e.target.checked));
                }}
              />
            ...
            {updateInfo && (
              <div className="update-result" role="status">
                {updateInfo.available ? (
                  <>
                    <p>
                      Update available: <strong>v{updateInfo.latest_version}</strong> (current: v
      ...
      {tab === 'about' && (
        <section role="tabpanel" aria-label="About" className="settings-section">
          ...
          <dl className="about-list">
            ...
            <dd>
              <a
                href="https://github.com/opensearch-project/dashboards-desktop"
                target="_blank"
                rel="noreferrer"
              >
                GitHub
            ...
            <dt>Electron</dt>
            <dd>{navigator.userAgent.match(/Electron\/([\d.]+)/)?.[1] ?? 'N/A'}</dd>
            <dt>Chrome</dt>
            <dd>{navigator.userAgent.match(/Chrome\/([\d.]+)/)?.[1] ?? 'N/A'}</dd>
          </dl>
        </section>
      )}
    </div>
