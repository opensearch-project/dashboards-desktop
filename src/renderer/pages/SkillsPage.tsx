  ...
  return (
    <div className="admin-page" role="region" aria-label="Skills & Agents">
      ...
      <div className="admin-tabs" role="tablist">
        <button role="tab" aria-selected={tab === 'skills'} className={`admin-tab ${tab === 'skills' ? 'active' : ''}`} onClick={() => setTab('skills')}>Skills ({skills.length})</button>
        <button role="tab" aria-selected={tab === 'agents'} className={`admin-tab ${tab === 'agents' ? 'active' : ''}`} onClick={() => setTab('agents')}>Agent Personas ({personas.length})</button>
      </div>

      {tab === 'skills' && (
        <section role="tabpanel" aria-label="Skills">
          {skills.length === 0 ? (
            <div className="empty-state" role="status"><p>No skills installed</p></div>
          ) : (
            <div className="plugin-grid" role="list">
              {skills.map(s => (
                <div key={s.name} className={`plugin-card ${!s.enabled ? 'plugin-disabled' : ''}`} role="listitem">
                  ...
                  <div className="plugin-actions">
                    <button className="btn-sm btn-danger-sm" onClick={async () => { await window.osd.skills.remove(s.name); load(); }}>Remove</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {tab === 'agents' && (
        <section role="tabpanel" aria-label="Agent Personas">
          {personas.length === 0 ? (
            <div className="empty-state" role="status"><p>No agent personas available</p></div>
          ) : (
            <div className="plugin-grid" role="list">
              {personas.map(p => (
                <div key={p.id} className={`plugin-card ${p.active ? 'plugin-card-active' : ''}`} role="listitem">
                  <div className="plugin-card-header">
                  ...
                  <div className="plugin-meta"><span>{p.skills.length} skills</span></div>
                  <div className="plugin-actions">
                    {!p.active && <button className="btn-primary btn-sm" onClick={() => switchAgent(p.id)}>Switch to this agent</button>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {installOpen && (
        <div className="dialog-overlay" role="presentation" onClick={e => { if (e.target === e.currentTarget) setInstallOpen(false); }}>
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="install-skill-title">
            <h2 id="install-skill-title">Install Skill</h2>
            <div className="form-group">
              <label htmlFor="skill-source">Package name or path</label>
              <input id="skill-source" value={installSource} onChange={e => setInstallSource(e.target.value)} placeholder="opensearch-dba or ./my-skill.ts" />
            </div>
