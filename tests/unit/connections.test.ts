...
const osInfoMock = vi.fn();
const esInfoMock = vi.fn();

vi.mock('@opensearch-project/opensearch', () => ({
  Client: vi.fn(() => ({ info: osInfoMock })),
}));

vi.mock('@elastic/elasticsearch', () => ({
  Client: vi.fn(() => ({ info: esInfoMock })),
}));
...
beforeEach(() => {
  osInfoMock.mockReset();
  esInfoMock.mockReset();
});

describe('testConnection: OpenSearch', () => {
  it('returns success for healthy cluster', async () => {
    osInfoMock.mockResolvedValue({
      body: { cluster_name: 'test-cluster', version: { number: '2.12.0' } },
    });
...
describe('testConnection: Elasticsearch', () => {
  it('returns success for healthy cluster', async () => {
    esInfoMock.mockResolvedValue({ cluster_name: 'es-cluster', version: { number: '8.17.0' } });
    const result = await testConnection({
  ...
  it('passes API key auth', async () => {
    esInfoMock.mockResolvedValue({ cluster_name: 'c', version: { number: '8.17.0' } });
    await testConnection({
...
describe('testConnection: failure modes', () => {
  it('returns error when client throws (unreachable)', async () => {
    osInfoMock.mockRejectedValue(new Error('connect ECONNREFUSED 127.0.0.1:1'));
    const result = await testConnection({
  ...
  it('returns error on auth failure', async () => {
    osInfoMock.mockRejectedValue(new Error('Response Error: 401 Unauthorized'));
    const result = await testConnection({
  ...
  it('returns error on timeout', async () => {
    esInfoMock.mockRejectedValue(new Error('Request timed out'));
    const result = await testConnection(
