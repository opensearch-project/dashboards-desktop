...
function buildFrame(eventType: string, payload: unknown, messageType = 'event'): Uint8Array {
  ...
  view.setUint32(8, 0); // prelude CRC (skipped in parser)
  buf.set(headers, 12);
  buf.set(payloadBytes, 12 + headers.length);
  view.setUint32(totalLen - 4, 0); // message CRC (skipped in parser)
  return buf;
}

/** Encode event-stream headers: [1B name_len][name][1B type=7][2B val_len][value] */
...
// Stub SigV4 — pass through unchanged
...
describe('BedrockProvider: binary event-stream parsing', () => {
  ...
  it('streams tool calls from binary frames', async () => {
    ...
    const provider = new BedrockProvider('us-east-1', noSign);
    const chunks = await collect(provider, 'anthropic.claude-sonnet-4-20250514-v1:0', 'health check');
